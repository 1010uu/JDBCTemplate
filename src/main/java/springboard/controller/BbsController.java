...
/*
 기본 패키지로 설정한 곳에 컨트롤러를 선언하면 요청이 들어왔을 때
...
public class BbsController {
	...
	/*
	 @Autowired
	 	...
	 	싶을 때 사용한다. 타입을 기반으로 자동 주입되며, 만약 해당 타입의
	 	빈이 존재하지 않으면 에러가 발생되어 서버를 시작할 수 없다.
	 	-생성자, 멤버변수, 메서드(setter)에 적용할 수있다.
	 	-타입을 이용해 자동으로 프로퍼티 값을 설정한다.
	 	-해당 어노테이션은 멤버변수에만 적용할 수있다. 매서드 내의 
	 	지역변수에는 사용할 수 없다.
	...
	public void setTemplate(JdbcTemplate template) {
		...
		//JdbcTemplate를 해당 프로그램 전체에서 사용하기 위한 설정(static타입)
		JdbcTemplateConst.template = this.template;
	}
	
	/*
	 멤버 변수로 선언하여 클래스에서 전역적으로 사용할 수 있다. 해당 클래스의
	...
	//전송방식까지 post이므로 value, method 같이 기술해서 매핑
	...
	public String writeAction(Model model, HttpServletRequest req, SpringBbsDTO springBbsDTO) {
		...
		/*
		 글쓰기 페이지에서 전송된 모든 폼 값은 springBbsDTO객체를 통해
		 한번에 받을 수 있다. Spring에서는 커맨드 객첼르 통해 이와 같은 
		 처리를 할 수있다.
		 */
		...
		//redirect : 뷰를 반환하지 않고, 지정된 URL(요청명)으로 이동한다.
	...
	public String password(Model model, HttpServletRequest req) {
		...
		//일련번호는 컨트롤러에서 파라미터를 받은 후 Model에 저장하여 View로 전달
